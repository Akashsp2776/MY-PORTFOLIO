    ...
    <section id="experience" ref={ref} className="py-section relative">
      <div className="px-container mx-auto max-w-7xl">
        <motion.div initial={{ opacity: 0, y: 30, filter: 'blur(8px)' }} animate={inView ? { opacity: 1, y: 0, filter: 'blur(0px)' } : {}} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}>
          <span className="eyebrow mb-6"><span className="h-px w-8 bg-neutral-600" />Internship Experience</span>
          <h2 className="section-title text-balance">Where I've put theory into practice.</h2>
          <p className="mt-4 max-w-2xl text-neutral-400">Real-world software engineering experience from internships where I contributed to production code, worked in agile teams, and shipped features that mattered.</p>
        </motion.div>
                  ...
                  <motion.div style={{ transformStyle: 'preserve-3d' }} className="perspective-1000">
                    <TiltCard maxTilt={3} scale={1.01} className="card-premium group relative overflow-hidden p-7">
                      <div className="relative flex items-start justify-between" style={{ transform: 'translateZ(30px)' }}>
                        <span className="grid h-12 w-12 place-items-center rounded-xl border border-[#2a2a2a] bg-[#161616] text-white transition-colors group-hover:border-[#3a3a3a]">
                          <Briefcase className="h-5 w-5" />
                        </span>
                        <span className="rounded-full border border-[#2a2a2a] bg-[#161616] px-3 py-1 text-xs font-medium text-neutral-400">{exp.type}</span>
                      </div>
                      <div className="relative mt-5" style={{ transform: 'translateZ(20px)' }}>
                        ...
                        <div className="mt-1 flex items-center gap-2 text-sm">
                          <span className="font-medium text-neutral-200">{exp.company}</span>
                      ...
                      <ul className="relative mt-5 space-y-2.5" style={{ transform: 'translateZ(15px)' }}>
                        {exp.highlights.map((h) => (
                          <li key={h} className="flex items-start gap-2.5 text-sm leading-relaxed text-neutral-400">
                      ...
                      <div className="relative mt-5 flex items-center gap-2 border-t border-[#2a2a2a] pt-4" style={{ transform: 'translateZ(10px)' }}>
                        ...
                        <span className="text-xs text-neutral-500">Real-world impact · Agile team · Production code</span>
                      </div>
                    </TiltCard>
                  </motion.div>
                </div>
