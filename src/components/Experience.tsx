      ...
      <div className="px-container mx-auto max-w-7xl">
        ...
        <div className="mt-14 grid gap-6 lg:grid-cols-2">
          {experiences.map((exp, i) => (
            <motion.div key={exp.id} initial={{ opacity: 0, y: 30, rotateX: -12 }} animate={inView ? { opacity: 1, y: 0, rotateX: 0 } : {}} transition={{ duration: 0.6, delay: 0.1 + i * 0.12, ease: [0.22, 1, 0.36, 1] }} style={{ transformStyle: 'preserve-3d' }} className="perspective-1000">
              <TiltCard maxTilt={9} scale={1.03} className="gradient-border group relative h-full overflow-hidden p-7 transition-shadow duration-300 hover:card-3d-shadow-hover">
              ...
              <div className="relative flex items-start justify-between" style={{ transform: 'translateZ(30px)' }}>
                <span className="grid h-12 w-12 place-items-center rounded-xl" style={{ background: `${exp.color}20`, border: `1px solid ${exp.color}40` }}><Briefcase className="h-5 w-5" style={{ color: exp.color }} /></span>
              ...
              <div className="relative mt-5" style={{ transform: 'translateZ(20px)' }}>
                ...
                <div className="mt-1 flex items-center gap-2 text-sm"><span className="font-medium text-slate-200">{exp.company}</span><span className="text-slate-600">·</span><span className="font-mono text-xs text-slate-400">{exp.period}</span></div>
              </div>
              <ul className="relative mt-5 space-y-2.5" style={{ transform: 'translateZ(15px)' }}>
                {exp.highlights.map((h) => (<li key={h} className="flex items-start gap-2.5 text-sm leading-relaxed text-slate-400"><span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full" style={{ background: exp.color }} />{h}</li>))}
              </ul>
              </TiltCard>
            </motion.div>
          ))}
