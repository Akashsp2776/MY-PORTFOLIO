      ...
      <div className="px-container mx-auto max-w-7xl">
        ...
        <div className="mt-16 relative">
          ...
          <div className="space-y-10">
            {education.map((edu, i) => (
              <motion.div key={edu.id} initial={{ opacity: 0, y: 30, rotateX: -10 }} animate={inView ? { opacity: 1, y: 0, rotateX: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 + i * 0.12, ease: [0.22, 1, 0.36, 1] }} style={{ transformStyle: 'preserve-3d' }} className={`relative flex md:items-center perspective-1000 ${i % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                ...
                <div className={`ml-12 md:ml-0 md:w-1/2 ${i % 2 === 0 ? 'md:pr-12 md:text-right' : 'md:pl-12'}`}>
                  <TiltCard maxTilt={8} scale={1.02} className="gradient-border h-full p-6 transition-shadow duration-300 hover:card-3d-shadow-hover">
                    <div className={`flex items-center gap-2 ${i % 2 === 0 ? 'md:justify-end' : ''}`} style={{ transform: 'translateZ(25px)' }}><GraduationCap className="h-4 w-4" style={{ color: edu.color }} /><span className="font-mono text-xs uppercase tracking-wider" style={{ color: edu.color }}>{edu.period}</span></div>
                    ...
                    <div className={`mt-3 flex flex-wrap gap-2 ${i % 2 === 0 ? 'md:justify-end' : ''}`} style={{ transform: 'translateZ(10px)' }}>{edu.highlights.map((h) => (<span key={h} className="chip">{h}</span>))}</div>
