    ...
    >
      ...
      <div className="relative h-[40vh] min-h-[300px] overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-end">
          <div className="px-container mx-auto w-full max-w-5xl pb-10">
            <button
              onClick={onBack}
              className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-slate-300 transition-all hover:border-white/20 hover:text-white"
            >
      ...
      <div className="px-container mx-auto max-w-5xl py-16">
        <div className="grid gap-12">
          {/* Overview */}
          <section>
            <h2 className="mb-4 flex items-center gap-2 font-display text-2xl font-semibold text-white">
          ...
          <section>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {project.features.map((f) => (
                <div key={f} className="flex items-start gap-3 rounded-xl border border-white/5 bg-white/5 p-4">
                ...
                {project.architecture.split('→').map((node, idx, arr) => (
                  <div key={idx} className="flex items-center gap-3">
                    <div className="rounded-xl border border-white/10 bg-white/5 px-4 py-3">
                      <span className="text-sm font-medium text-slate-200">{node.trim()}</span>
                    </div>
                    {idx < arr.length - 1 && (
            ...
            <div className="space-y-3">
              {project.timeline.map((t, i) => (
                <div key={i} className="flex items-center gap-4">
                  <div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-gradient-to-br from-blue-600 to-purple-600 text-xs font-bold text-white">
                    {i + 1}
                  </div>
                  <div className="flex-1 gradient-border p-4">
                    <span className="font-medium text-white">{t.phase}</span>
                    <span className="ml-3 font-mono text-xs text-slate-500">{t.duration}</span>
                  </div>
                </div>
              ))}
            </div>
              ...
              {project.challenges.map((c, i) => (
                <div key={i} className="gradient-border p-6">
                  <div className="mb-3">
                    ...
                    <p className="text-sm text-slate-300">{c.challenge}</p>
                  </div>
                  <div>
                    <span className="mb-1 block text-xs font-medium uppercase tracking-wider text-emerald-400">
                      Solution
                    </span>
          ...
          <section>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {project.learnings.map((l) => (
                <div key={l} className="flex items-start gap-3 rounded-xl border border-white/5 bg-white/5 p-4">
          ...
          <section>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {[1, 2, 3, 4].map((n) => (
                <div
                  key={n}
                  className="flex aspect-video items-center justify-center rounded-xl border border-white/5 bg-white/5 text-slate-600"
                >
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-t border-white/5 pt-8">
            <button
              onClick={onBack}
              className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-5 py-3 text-sm font-semibold text-slate-200 transition-all hover:border-white/20 hover:text-white"
            >
              <ArrowLeft className="h-4 w-4" />
              Back to Portfolio
            </button>
