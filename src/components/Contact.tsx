...
import { Mail, Phone, MapPin, Github, Linkedin, Send, CheckCircle2, Map, Loader2, AlertCircle, Sparkles } from 'lucide-react'
...
export default function Contact() {
  ...
  return (
    <section id="contact" ref={ref} className="py-section relative overflow-hidden">
      ...
      <div className="pointer-events-none absolute -left-32 top-1/4 h-80 w-80 rounded-full bg-blue-600/10 blur-[100px]" />
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-5">
          ...
          <motion.form onSubmit={handleSubmit} initial={{ opacity: 0, x: 30, filter: 'blur(6px)' }} animate={inView ? { opacity: 1, x: 0, filter: 'blur(0px)' } : {}} transition={{ duration: 0.7, delay: 0.2, ease: [0.22, 1, 0.36, 1] }} className="gradient-border relative overflow-hidden p-7 lg:col-span-3" noValidate>
            ...
            <div className="relative mb-6 flex items-center gap-2">
              ...
              <span className="font-mono text-xs uppercase tracking-wider text-slate-400">Send a message</span>
            </div>
            ...
            {status === 'sent' && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="relative mt-5 flex items-center gap-2 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-300">
                <CheckCircle2 className="h-4 w-4 shrink-0" />
                <span>Message sent successfully! Thank you for contacting me. I'll get back to you as soon as possible.</span>
              </motion.div>
            )}
            {status === 'error' && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="relative mt-5 flex items-center gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-300">
